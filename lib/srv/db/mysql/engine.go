...
func (e *Engine) connect(ctx context.Context, sessionCtx *common.Session) (*client.Conn, error) {
	...
	case sessionCtx.Server.IsCloudSQL():
		// For Cloud SQL MySQL we reset database user password for each connect
		// since there's no IAM auth. Thus, acquire a lock to make sure all
		// connection attempts to the same database and user are serialized.
